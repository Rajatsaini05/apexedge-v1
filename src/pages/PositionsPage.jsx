...
// deleteTrade and updateTrade come as PROPS from App.jsx (which owns useTrades).
// Never call useTrades() here — that would create a second instance with its
// own state and the UI would not update when you delete/edit.
...
const EditModal = ({ trade, onSave, onClose, busy }) => {
  ...
  const F = ({ label, k, type = 'text', ph = '' }) => (
    <div>
      <label style={{ fontSize:11, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'.6px', display:'block', marginBottom:5, fontFamily:'var(--fm)' }}>{label}</label>
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.75)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <Card className="fade-up" style={{ width:520, maxWidth:'100%', maxHeight:'90vh', overflowY:'auto', padding:24 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          ...
          <F label="Entry Price"  k="entryPrice" type="number" />
          <F label="Exit Price"   k="exitPrice"  type="number" ph="Leave blank if open" />
          <F label="Lot Size"     k="lots"       type="number" />
          <div/>{/* spacer */}
          <F label="Entry Time"   k="entryDate"  type="datetime-local" />
          <F label="Exit Time"    k="exitDate"   type="datetime-local" />
          <F label="Stop Loss"    k="sl"         type="number" ph="Optional" />
          <F label="Take Profit"  k="tp"         type="number" ph="Optional" />
        </div>
        ...
        <div style={{ display:'flex', gap:8, justifyContent:'flex-end', marginTop:18 }}>
          ...
          <Btn size="sm" onClick={handleSave} disabled={busy}>
            {busy ? <div className="spin" style={{ width:12, height:12, border:'2px solid rgba(255,255,255,.3)', borderTopColor:'#fff', borderRadius:'50%' }}/> : <Check size={13}/>}
            Save Changes
          </Btn>
        </div>
      </Card>
...
const PositionsPage = ({ trades, setTrades, addTrade, deleteTrade, updateTrade, setPage }) => {
  ...
  const [showEdit, setShowEdit] = useState(null); // trade being edited
  ...
  const handleAddManual = async () => {
    ...
    const raw  = !isNaN(xp) && xp > 0
      ? (form.side==='buy' ? (xp-ep)*lots*100 : (ep-xp)*lots*100) : null;
    const pnl    = raw !== null ? +raw.toFixed(2) : null;
    const status = pnl===null ? 'open' : pnl>0 ? 'win' : pnl<0 ? 'loss' : 'be';
  ...
  const handleDelete = async (id) => {
    ...
    try {
      if (deleteTrade) {
        // Uses the prop from App.jsx — updates the correct shared state
  ...
  const handleUpdate = async (updates) => {
    ...
    try {
      if (updateTrade) {
        ...
        await updateTrade(showEdit.id, updates);
      } else {
        setTrades(p => p.map(t => t.id === showEdit.id ? { ...t, ...updates } : t));
      }
      setShowEdit(null);
  ...
  const Th = ({ col, label }) => (
    <th onClick={() => setSort(s => ({ col, asc: s.col===col ? !s.asc : false }))}
  ...
  // ── Add Trade modal (shared between empty + populated states) ─────────────
  ...
  if (!trades.length) return (
    <div style={{ flex:1, display:'flex', flexDirection:'column' }}>
      ...
      <EmptyState icon={List} title="No positions yet"
        desc="Add trades manually using the button above, or import a broker CSV."
        cta="Import CSV" onCta={()=>setPage('import')}/>
      <AddModal/>
    </div>
  );
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead style={{ position:'sticky', top:0, zIndex:1 }}>
              <tr>
                ...
                <Th col="entryDate"  label="Entry"/>
                <Th col="exitDate"   label="Exit"/>
              ...
              {filtered.map((t,i) => (
                <tr key={t.id}
                  ...
                  <td style={{ padding:'9px 14px' }}>
                    <span style={{ fontSize:11, color:t.side==='buy'?'var(--emerald)':'var(--rose)', fontFamily:'var(--fm)', fontWeight:500 }}>{t.side==='buy'?'▲ L':'▼ S'}</span>
                  </td>
                  ...
                  <td style={{ padding:'9px 10px', textAlign:'center' }}>
                    <div style={{ display:'flex', gap:4, justifyContent:'center' }}>
                      ...
                      <button
                        ...
                        title="Edit trade"
                        style={{ background:'none', border:'none', color:'var(--muted)', cursor:'pointer', padding:5, borderRadius:5, transition:'color .12s' }}
      ...
      {/* Detail strip */}
      {sel && (
        <div className="fade-in" style={{ borderTop:'1px solid var(--line2)', padding:'14px 20px', background:'rgba(99,102,241,.03)', display:'flex', gap:24, alignItems:'flex-start', flexWrap:'wrap' }}>
          <div style={{ flex:1 }}>
            ...
            <div style={{ display:'flex', gap:16, fontSize:12, fontFamily:'var(--fm)', color:'var(--sub)', flexWrap:'wrap' }}>
              {[['Entry',sel.entryPrice?.toFixed(5)],['Exit',sel.exitPrice?.toFixed(5)||'—'],['Lots',sel.lots],['Duration',sel.duration],['TP',sel.tp?.toFixed(5)||'—'],['SL',sel.sl?.toFixed(5)||'—'],['Source',sel.source||'csv']].map(([k,v])=>(
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            ...
            <button onClick={()=>setSel(null)} style={{ background:'none', border:'none', color:'var(--muted)', cursor:'pointer', padding:4 }}><X size={16}/></button>
          </div>
        </div>
      )}
